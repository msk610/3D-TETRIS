...
public class MainGame implements Screen {
    ...
    //Constructor
    public MainGame(Game game, AssetLoader assets, ButtonLoader buttons, Button music,Button sound){
        this.game = game;
        this.assets = assets;
    ...
    public void show() {
        ...
        cam.near = 1f;
        cam.far = 300f;
        cam.translate(10f, 0, 12f);
        cam.update();
        camController = new CameraInputController(cam);
        camController.target = new Vector3(13f, 0f, 14f);
    ...
    public void render(float delta) {
        ...
        if(!gameOver){
            ...
            if(!play.isChecked()){
                ...
                timeLasped += elasped;
                keyboard.interact(sound,clock,play,assets,pieces.get(index),fixed);
                ...
                if(timeLasped >= interval){
                    ...
                    if(!check){
                        ...
                        if(!gameOver) {
                            ...
                            boolean scoredPoints = helper.clearBlocks(fixed);
                            while(scoredPoints) {
                                if (!sound.isChecked()) {
                                    assets.score.play();
                                }
                                scoredPoints = helper.clearBlocks(fixed);
                            }
